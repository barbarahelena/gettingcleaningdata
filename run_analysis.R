...
rewrite_labels <- function(x){str_remove(x, pattern="[0-9]+[ ]+")} # Function to remove numbers and spaces
...
## Checks before binding test and train sets
...
## Binding test and train sets
testtrain <- rbind(test, train)

# Selecting variables with means or standard deviations
dataSet <- testtrain %>% 
  select(id, lab, contains('mean()') | contains('std'))

## Tidying column names
colnames(dataSet) <- str_remove_all(colnames(dataSet), '\\(\\)') # Remove all parentheses from var names
colnames(dataSet) <- str_replace_all(colnames(dataSet), '-', '_') # Replace dashes with underscores
colnames(dataSet) # Check resulting var names
...
dataSet$activity <- ifelse(dataSet$lab %in% act$n, act$act[match(dataSet$lab, act$n)], NA) # Match labels with activity descriptions
